 ...
 * When the number of elements to sort is at most: TSODLULS_MAX_THRESH for initial sort or TSODLULS_MAX_THRESH_SEQUEL for radix sequel, we use insertion sort.
...
//  #define TSODLULS_MAX_THRESH_SEQUEL 2
  ...
  while(1){
    ...
    i_last_octet = 0;
    b_decreasing = 0;
    i_counter = current_instance.i_offset_first;
    //we count the bytes with a certain value
    if(current_instance.b_copy){
      for(; i_counter <= current_instance.i_offset_last; ++i_counter){
        i_current_octet = arr_elements_copy[i_counter].s_key[current_instance.i_depth];
      ...
      for(; i_counter <= current_instance.i_offset_last; ++i_counter){
        i_current_octet = arr_elements[i_counter].s_key[current_instance.i_depth];
        ++arr_counts[i_current_octet];
        if(i_current_octet < i_last_octet){
          b_decreasing = 1;
          ++i_counter;
