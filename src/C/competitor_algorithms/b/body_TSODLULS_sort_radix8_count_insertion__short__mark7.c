...
//  #define TSODLULS_MAX_THRESH 5
  ...
  size_t coarse_logarithmic_bound = 0;
  size_t auxiliary_variable_for_coarse_logarithmic_bound = 0;
  size_t biggest_instance_size = 0;
  size_t biggest_instance_offset = 0;
  size_t this_round_first_instance_offset = 0;
  t_TSODLULS_radix_instance swap_instance;
  ...
  //Since we use Hoare's rule, a logarithmic number is sufficient:
  //up to 256 parts you need at least p*2 elements,
  //up to 511 parts, you need at least 254*2 + (p-255)*4 elements,
  //up to 766 parts, you need at least 254*2 + 254*4 + (p-510)*8 elements, etc.
  //Thus a coarse bound is 256*i if the number of elements is <= 256*(2^i).
  //Exercise: replace 2 by (TSODLULS_MAX_THRESH + 1) in formulas above.
  do{
    size_t i = 1;
  ...
  while(1){
    ...
    biggest_instance_size = 0;
    biggest_instance_offset = 0;
    this_round_first_instance_offset = i_current_instance;
      ...
      if(i_max_length > current_instance.i_depth + 1){
        for(int i = 0; i < 256; ++i){
          if(arr_counts[i] > TSODLULS_MAX_THRESH){//nothing to do for at most TSODLULS_MAX_THRESH elements, result is in original array
            ...
            if(
              biggest_instance_size
              < arr_instances[i_current_instance].i_offset_last
              - arr_instances[i_current_instance].i_offset_first
            ){
              biggest_instance_size =
                arr_instances[i_current_instance].i_offset_last
                - arr_instances[i_current_instance].i_offset_first
              ;
              biggest_instance_offset = i_current_instance;
            }
            ++i_current_instance;
          }
    ...
    //Hoare's rule, we swap instances if needed to avoid the biggest part
    if(
      biggest_instance_offset != 0
      && biggest_instance_offset != this_round_first_instance_offset
    ){
      swap_instance = arr_instances[this_round_first_instance_offset];
      arr_instances[this_round_first_instance_offset] = arr_instances[biggest_instance_offset];
      arr_instances[biggest_instance_offset] = swap_instance;
    }
