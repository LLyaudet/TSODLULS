 ...
 */
//int TSODLULS_Tim_sort__long__mark1(
...
//){
   ...
   */
  int TSODLULS_merge_two_runs__long(
    t_TSODLULS_Tim_sort_merge_state* merge_state,
    size_t i_first_index_to_merge
  ){
    #include "./body_TSODLULS_merge_two_runs__long.c"
  }//end function TSODLULS_merge_two_runs__long()


  size_t i_run_min_number_of_elements;
  //[lower, higher[
  t_TSODLULS_sort_element* p_current_lower_element = arr_elements;
  t_TSODLULS_sort_element* p_current_higher_element = p_current_lower_element + i_number_of_elements;
  t_TSODLULS_Tim_sort_merge_state merge_state;
  t_TSODLULS_sort_element temp_cell;
  size_t i__compare;
  ...
  merge_state.arr_elements_copy = calloc(merge_state.i_max_number_of_elements_copy, sizeof(t_TSODLULS_sort_element));
  ...
  do{
    ...
    do{
      t_TSODLULS_sort_element* p_current_element = p_current_lower_element + 1;
      i_current_run_number_of_elements = 1;
      if(p_current_element >= p_current_higher_element){
        break;
      }
      i_current_run_number_of_elements = 2;
      TSODLULS_set_min_length__macraff(i_max__compare, p_current_lower_element, p_current_element);
      for(i__compare = 0; i__compare < i_max__compare; ++i__compare){
        i_result__compare = ((int)(p_current_lower_element->s_key[i__compare])) - ((int)(p_current_element->s_key[i__compare]));
      ...
      if(i_result__compare >= 0){
        ...
        for(
          ...
          TSODLULS_set_min_length__macraff(i_max__compare, p_current_element - 1, p_current_element);
          for(i__compare = 0; i__compare < i_max__compare; ++i__compare){
            i_result__compare = ((int)((p_current_element - 1)->s_key[i__compare])) - ((int)(p_current_element->s_key[i__compare]));
            if(i_result__compare != 0){
              break;
            }
          }
    ...
    if(b_descending_run){
      t_TSODLULS_sort_element* p_lower_element_for_swap = p_current_lower_element;
      t_TSODLULS_sort_element* p_higher_element_for_swap = p_current_lower_element + i_current_run_number_of_elements - 1;
      do{
    ...
    if(i_current_run_number_of_elements < i_run_min_number_of_elements){
      t_TSODLULS_sort_element* const start_ptr = p_current_lower_element;
      t_TSODLULS_sort_element* end_ptr = p_current_lower_element + i_run_min_number_of_elements - 1;
      ...
      i_result__compare = TSODLULS_binary_insertion_sort_stable__long__mark1(
          start_ptr,
