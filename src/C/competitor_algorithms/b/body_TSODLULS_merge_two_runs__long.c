...
#define TSODLULS_copy_and_inc(insertion_point, from, length, offset) \
  {\
    memcpy(insertion_point, from, offset * sizeof(t_TSODLULS_sort_element));\
...
#define TSODLULS_move_and_inc(insertion_point, from, length, offset) \
  {\
    memmove(insertion_point, from, offset * sizeof(t_TSODLULS_sort_element));\
    insertion_point += offset;\
    from += offset;\
    length -= offset;\
  }
...
//){
   ...
   */
  int TSODLULS_gallop_left__long(
    t_TSODLULS_sort_element* p_element,
    t_TSODLULS_sort_element* arr_sorted_elements,
    size_t i_number_of_elements,
    size_t i_hint_offset
  ){
    #include "./body_TSODLULS_gallop_left__long.c"
  }//end function TSODLULS_gallop_left__long()
  ...
  int TSODLULS_gallop_right__long(
    ...
    #include "./body_TSODLULS_gallop_right__long.c"
  }//end function TSODLULS_gallop_right__long()

  t_TSODLULS_sort_element* p_base_1;
  t_TSODLULS_sort_element* p_base_2;
  size_t i_length_1;
  size_t i_length_2;
  size_t i_offset;
  size_t i__compare;
  ...
  i_offset = TSODLULS_gallop_right__long(
  ...
  i_offset = TSODLULS_gallop_left__long(
  ...
  if(i_length_1 <= i_length_2){
    ...
    t_TSODLULS_sort_element* p_current_insertion_point = p_base_1;
    memcpy(merge_state->arr_elements_copy, p_base_1, i_length_1 * sizeof(t_TSODLULS_sort_element));
    ...
    *p_current_insertion_point++ = *p_base_2++;
    ...
    while(1){
      ...
      //normal merge
      while(1){
        TSODLULS_set_min_length__macraff(i_max__compare, p_base_1, p_base_2);
        for(i__compare = 0; i__compare < i_max__compare; ++i__compare){
          i_result__compare = ((int)(p_base_1->s_key[i__compare])) - ((int)(p_base_2->s_key[i__compare]));
    ...
    t_TSODLULS_sort_element* p_true_base_1;
    t_TSODLULS_sort_element* p_true_base_2;
    size_t i_min_gallop = merge_state->i_min_gallop;
    t_TSODLULS_sort_element* p_current_insertion_point = p_base_2 + i_length_2 - 1;
    memcpy(merge_state->arr_elements_copy, p_base_2, i_length_2 * sizeof(t_TSODLULS_sort_element));
